...
double map_prob(int position[2], char map[9][9]){
    srand(144);
    ...
    double avg_steps = total_steps/1000.0;
    double success_perc = successful_runs/10.0;
    //printf("average steps: %lf, percentage success: %lf", avg_steps, success_perc);
    return avg_steps, success_perc;
}
...
int main(){
    ...
    for(int i = 0; i < 9; i++){ //displaying the map
        for(int j = 0; j < 9; j++){
            ...
            percentage, average_steps = map_prob(position, map);
            perc[i][j] = percentage;
